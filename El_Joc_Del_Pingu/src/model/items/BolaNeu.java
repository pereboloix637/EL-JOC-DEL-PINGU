...
public class BolaNeu extends Item {
    ...
    // Getters i Setters
    public int getRetrocedir() { return retrocedir; }
    public void setRetrocedir(int retrocedir) { this.retrocedir = retrocedir; }
    ...
    public boolean llençarBola(Jugador atacant, Jugador objectiu) {
        if (usar()) {
            ...
            System.out.println(atacant.getNickname() + " ha llençat una bola de neu a "
                    + objectiu.getNickname() + "! Retrocedeix "
                    + retrocedir + " caselles fins a la posició " + novaPos + ".");
            return true;
        }
        return false;
    }
    ...
    public boolean llençarBolaAleatoria(Jugador atacant, ArrayList<Jugador> jugadors) {
        ...
        for (Jugador j : jugadors) {
            if (!j.equals(atacant)) rivals.add(j);
        }
        if (rivals.isEmpty()) {

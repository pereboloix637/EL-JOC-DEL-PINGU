...
public class Inventari {
    ...
    public int contarTipus(Class<? extends Item> tipus) {
        int total = 0;
        for (Item i : llista) {
            if (tipus.isInstance(i)) total += i.getQuantitat();
        }
        return total;
    }

    public int getBoles()         { return contarTipus(BolaNeu.class); }
    public int getPeixos()        { return contarTipus(Peix.class); }
    public int getDausEspecials() { return contarTipus(Dau.class); }

    // Retorna el primer ítem disponible del tipus indicat, o null
    public Item obtenirPrimer(Class<? extends Item> tipus) {
        for (Item i : llista) {
            if (tipus.isInstance(i) && i.getQuantitat() > 0) return i;
        }
        return null;
    }
    ...
    // Afegeix un ítem respectant el límit màxim (hardcoded per tipus). Retorna les unitats afegides realment
    public int afegirItem(Item item) {
        int max;
        if      (item instanceof BolaNeu) max = 6;
        else if (item instanceof Peix)    max = 2;
        else if (item instanceof Dau)     max = 3;
        else {
    ...
    public void eliminarItem(Item item) {
        if (llista.remove(item)) System.out.println("S'ha eliminat: " + item.getNom());
        else System.out.println("L'ítem " + item.getNom() + " no és a l'inventari.");
    }

    // Retorna l'ítem si existeix, null altrament
    public Item obtenirItem(Item item) {
        for (Item i : llista) { if (i.equals(item)) return i; }
        return null;
    }

    // Llença un ítem sense usar-lo
    public void tirarItem(Item item) {
        if (llista.remove(item)) System.out.println("Has llençat: " + item.getNom());
        else System.out.println("L'ítem " + item.getNom() + " no és a l'inventari.");
    }
    ...
    public boolean usarItem(Item item) {
        if (llista.contains(item)) {
            ...
            if (usat && item.getQuantitat() <= 0) llista.remove(item);
            return usat;
        }
    ...
    public String toString() {
        if (llista.isEmpty()) return "Inventari buit.";
        StringBuilder sb = new StringBuilder("Inventari:\n");
        for (Item i : llista) sb.append("  - ").append(i).append("\n");
        return sb.toString();
    }
}